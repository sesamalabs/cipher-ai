...
//
// Simbol yang dipakai: GC=F (COMEX Gold Futures) — ini kode yang benar-benar dikenali
// Yahoo Finance untuk harga emas ("XAUUSD=X" TIDAK ada di database mereka, itu sempat
// bikin error 404). GC=F adalah acuan harga gold paling umum dipakai di Yahoo Finance,
// selisihnya dengan harga spot XAUUSD di platform trading biasanya cuma beberapa dolar
// (basis futures vs spot) — cukup dekat untuk analisa teknikal, tapi tetap sanity-check
// entry-nya terhadap harga live di platform kamu sebelum eksekusi.
//
...
async function fetchYahooChart(interval, range) {
  ...
  if (!result) throw new Error("Yahoo Finance: data gold tidak ditemukan");
  ...
  for (let i = 0; i < ts.length; i++) {
    ...
    const l = q.low?.[i];
    const c = q.close?.[i];
    // lewati bar kosong (pasar tutup akhir pekan/malam hari libur dsb.)
...
// Ambil candle gold untuk timeframe tertentu, hasil ascending (lama -> baru)
...
// Harga terkini gold (candle terakhir dari data harian terbaru)
export async function fetchGoldQuote() {
  const candles = await fetchYahooChart("1d", "5d");
  if (candles.length === 0) throw new Error("Harga gold tidak tersedia (pasar mungkin tutup)");
