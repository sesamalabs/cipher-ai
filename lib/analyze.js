...
export async function analyzeSymbol(symbol, timeframe = "4H") {
  ...
  const prompt =
    "Kamu analis teknikal crypto yang disiplin dan jujur. Berdasarkan HANYA data berikut, " +
    "tentukan apakah ada setup LONG spot yang layak di " +
    symbol +
    " timeframe " +
    timeframe +
    ".\n\n" +
  ...
  if (!res.ok) {
    ...
    return { ok: false, note: "Gemini error " + res.status + ": " + t.slice(0, 200) };
  }
